...
function renderSupplyPlan(){
 ...
 const heading=document.createElement('h2');heading.textContent='Choose Your Supplies';root.append(heading);
 ...
 const groups=[...new Set([...suggested,...Object.keys(SUPPLY_GUIDE),...Object.keys(APPLIANCE_SUPPLY_GUIDE)])];
 const intro=document.createElement('p');intro.className='lead';intro.textContent=suggested.length?'Supplies related to your materials and appliances are marked below. Review the full lists and check what your project needs.':'Browse every supply list below. Add materials or appliances to your project to highlight likely matches.';root.append(intro);
 ...
 for(const group of groups){const guide=SUPPLY_GUIDE[group]||APPLIANCE_SUPPLY_GUIDE[group],card=document.createElement('section'),h=document.createElement('h3'),list=document.createElement('div');card.className='panel supply-guide-card';h.textContent=guide.label;list.className='supply-guide-list';card.append(h);if(suggested.includes(group)){const badge=document.createElement('span');badge.className='supply-match';badge.textContent='From your project';card.append(badge)}card.append(list);
  ...
  const selectGroup=document.createElement('button');selectGroup.type='button';selectGroup.className='btn outline supply-select-group';selectGroup.textContent='Select All in This List';selectGroup.addEventListener('click',()=>{list.querySelectorAll('input[type="checkbox"]').forEach(box=>{box.checked=true;checked[box.dataset.key]=true});saveChecks()});card.append(selectGroup);grid.append(card)}
 root.append(grid);updateCount();
 const actions=document.createElement('div');actions.className='supply-actions';
 const selectAll=document.createElement('button');selectAll.type='button';selectAll.className='btn outline';selectAll.textContent='Select All Supplies';selectAll.addEventListener('click',()=>{grid.querySelectorAll('input[type="checkbox"]').forEach(box=>{box.checked=true;checked[box.dataset.key]=true});saveChecks()});
 const clear=document.createElement('button');clear.type='button';clear.className='btn outline';clear.textContent='Clear Selection';clear.addEventListener('click',()=>{grid.querySelectorAll('input[type="checkbox"]').forEach(box=>{box.checked=false;checked[box.dataset.key]=false});saveChecks()});
 const add=document.createElement('button');add.type='button';add.className='btn primary';add.textContent='Add Selected to Project';const status=document.createElement('p');status.className='calc-note';status.setAttribute('role','status');add.addEventListener('click',()=>{const names=[...new Set([...grid.querySelectorAll('input[type="checkbox"]:checked')].map(box=>box.dataset.name))];if(!names.length){status.textContent='Select at least one supply first.';return}const cart=getCart();let added=0;for(const name of names){if(!cart.some(item=>item.name===name)){cart.push({name,qty:1,type:'Supplies'});added++}}saveCart(cart);status.textContent=added?`${added} supplies added to your Project Cart.`:'These supplies are already in your Project Cart.'});
 actions.append(selectAll,clear,add);root.append(actions,status);const note=document.createElement('p');note.className='calc-note';note.textContent='Your selections stay on this device. Check product compatibility and final quantities before ordering. No prices are shown.';root.append(note)
}
