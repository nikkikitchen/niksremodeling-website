...
function materialCalcLink(product){const kind=product.category==='Flooring'?'flooring':product.category==='Tile & Stone'?'tile':product.category==='Drywall'?'drywall':product.category==='Lumber'?'lumber':null;if(!kind)return null;return `/calculators.html?material=${encodeURIComponent(kind)}&product=${encodeURIComponent(product.name)}`}
function materialShopLink(item){const kind=item.type==='flooring'?(item.floorKind==='tile'?'tile':'flooring'):item.type==='drywall'?'drywall':item.type==='lumber'?'lumber':'';return kind?`/materials.html?type=${encodeURIComponent(kind)}`:'/materials.html'}
function applyMaterialShopFilter(){const type=new URLSearchParams(location.search).get('type');if(!type)return;const allowed={tile:['Tile & Stone'],flooring:['Flooring'],drywall:['Drywall'],lumber:['Lumber']}[type];if(!allowed)return;const products=document.querySelectorAll('.products .card');products.forEach(card=>{card.hidden=!allowed.includes(card.dataset.category)});const catalog=document.querySelector('.catalog-head');if(catalog){const note=document.createElement('p');note.className='calc-note';note.textContent=`Showing ${type==='tile'?'tile & stone':type} materials from your calculation. `;const clear=document.createElement('a');clear.href='/materials.html';clear.textContent='See all materials';note.append(clear);catalog.append(note)}}
...
function renderCalculators(){
 ...
 const params=new URLSearchParams(location.search),kind=params.get('material'),product=params.get('product');const selected={tile:'flooring',flooring:'flooring',drywall:'drywall',lumber:'lumber',paint:'paint',concrete:'concrete'}[kind];if(selected){document.getElementById('material-type').value=selected;if(kind==='tile'||kind==='flooring')document.getElementById('floor-kind').value=kind==='tile'?'tile':'floating';showMaterialFields()}if(product){const note=document.createElement('p');note.className='calc-note';note.textContent=`Planning quantity for ${product}. Add your measurements below.`;document.querySelector('.calc-workspace .panel').prepend(note)}
}
